...
class ControllerExtensionPaymentAssetPayments extends Controller {
	...
	public function confirm() {
		if ($this->session->data['payment_method']['code'] == 'assetpayments') {
			$this->load->model('checkout/order');

			$this->model_checkout_order->addOrderHistory($this->session->data['order_id'], $this->config->get('assetpayments_order_status_id'));
		}
	}
	...
	public function callback() {
		...
		if ($status == 1 && $sign == $signature) {
			$this->load->model('checkout/order');
			$this->model_checkout_order->addOrderHistory($order_id, $this->config->get('assetpayments_order_success_status_id'),'AssetPayments TransactionID: ' .$transactionId);
		} 
